...
class Block:
    def __init__(self, block_data: dict):
        ...
        self.__properties: dict[str, any] = self.__data["minecraft:block"][
            "description"
        ].get("properties")
...
@app.command()
def sounds(
    rp_path: Path = typer.Argument(
        None, help="Resource pack where block sounds are being defined"
    ),
...
def define(
    ...
    rp_path: Path = typer.Argument(
        None, help="The path to the resource pack of this block"
    ),
    rp_name: str = typer.Argument(
        "Resource Pack", help="The name of the resource pack for this block"
    ),
    flipbook: bool = typer.Option(
        False, help="Whether this block is a flipbook texture"
    ),
    variation: bool = typer.Option(
        False, help="If the block has multiple textures that variate"
    ),
):
    """
    ...
    if blocks_rp_file.exists():
        ...
        blocks_data = {"format_version": "1.19.30"}
    ...
    if block_textr_folder.exists():
        block_textures = [
            str(textr)[textr.find("textures") :]
            .replace(os.sep, "/")
            .replace(".png", "")
            for textr in block_textr_folder.rglob("*.png")
        ]
        if variation:
            terrain_texture_data["texture_data"][block.name] = {}
            ...
            for textr in block_textures:
                ...
                terrain_texture_data["texture_data"][block.name]["textures"][
                    "variations"
                ].append({"path": textr, "weight": weight})
            ...
            for textr in block_textures:
                ...
                face = input(
                    f"What face should the texture {short_name}, render on?: "
                ).lower()
                while face not in valid_faces:
                    print(
                        f"Invalid face, please try again and choose from {valid_faces}"
                    )
                    face = input(
                        f"What face should the texture {short_name}, render on?: "
                    )
        ...
        terrain_texture_data["texture_data"][block.name][
            "textures"
        ] = f"textures/custom/blocks/{block.name}"
    ...
    helpers.write_to_file(
        terrain_textures, terrain_texture_data
