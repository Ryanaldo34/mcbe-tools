...
@app.command()
def define(
    rp_folder: Path = typer.Argument(None, help="ABS path to the resource pack"),
    entity_file: Path = typer.Argument(
        None, help="ABS path to the entity behavior file"
    ),
    ...
    anim: str = typer.Option(
        "", help="Specify name of the animation file for the entity"
    ),
    ac: str = typer.Option(
        "", help="Specify name of animation controller file for the entity"
    ),
    geo: str = typer.Option("", help="Specify a geometry for the entity"),
    material: Optional[list[str]] = typer.Option(
        None, help="Add a material to the entity"
    ),
    ...
    try:
        ...
        if template:
            ...
            builder.build_self(
                entity_file, identifier
            )  # builds out the template file to json

        entity_data = data_from_file(entity_file)
        behaviors = EntityBehaviors(entity_data)
        name = behaviors.real_name
        geo_path = (
            rp_folder.joinpath("models", "entity", f"{name}.geo.json")
            if not geo
            else rp_folder.joinpath("models", "entity", f"{geo}.geo.json")
        )
        anim_file = (
            rp_folder.joinpath("animations", f"{anim}.animation.json")
            if anim
            else rp_folder.joinpath("animations", f"{name}.animation.json")
        )
        ac_file = (
            ac
            if ac
            else rp_folder.joinpath(
                "animation_controllers", f"{name}.animation_controllers.json"
            )
        )
        texture_folder_path = rp_folder.joinpath("textures", "entity", f"{name}")
        texture_path = (
            rp_folder.joinpath("textures", "entity", f"{name}.png")
            if not texture_folder_path.exists()
            else texture_folder_path
        )
        if texture:
            texture_path = rp_folder.joinpath("textures", "entity", f"{texture}.png")
        if dummy:
            geo_object = client_entity.geo.Geometry(
                data_from_file(geo_path), dummy=True
            )
        ...
        if geo_data is None and geo_req:
            raise MissingGeometryError(
                "The entity is missing a required geometry definition!"
            )
        ...
        if entity.has_default_rc:
            ...
            render_controller = RenderController(
                f"controller.render.{entity.name}", arrays, materials
            )
        ...
        print(
            f"{Back.BLACK}{Style.BRIGHT}{Fore.RED}FATAL ERROR:\n",
            f"{Back.RESET}",
            f"The entity in {entity_file} could not be defined due to a missing animation controller requirement!",
            Style.RESET_ALL,
        )
        print(
            f"{Style.DIM}{Fore.YELLOW}Troubleshooting:\n",
            "+Is the file named entity_name.animation_controllers.json?\n",
            "+Is the the file saved in RP/animation_controllers",
            Style.RESET_ALL,
        )
        ...
        print(
            f"{Back.BLACK}{Style.BRIGHT}{Fore.RED}FATAL ERROR:\n",
            f"{Back.RESET}",
            f"The entity in {entity_file} could not be defined due to a missing animation requirement!",
            Style.RESET_ALL,
        )
        print(
            f"{Style.DIM}{Fore.YELLOW}Troubleshooting:\n",
            "+Is the file named entity_name.animation.json?\n",
            "+Is the the file saved in RP/animations?",
            Style.RESET_ALL,
        )
        raise typer.Abort() from exc
        ...
        print(
            f"{Back.BLACK}{Style.BRIGHT}{Fore.RED}FATAL ERROR:\n",
            f"{Back.RESET}",
            f"The entity in {entity_file} could not be defined due to a missing a geometry definition (model)!",
            Style.RESET_ALL,
        )
        print(
            f"{Style.DIM}{Fore.YELLOW}Troubleshooting:\n",
            "+Is the file named entity_name.geo.json?\n",
            "+Is the the file saved in RP/models/entity",
            Style.RESET_ALL,
        )
        raise typer.Abort() from exc
        ...
        print(
            f"{Back.BLACK}{Style.BRIGHT}{Fore.RED}FATAL ERROR:\n",
            f"{Back.RESET}",
            f"The entity in {entity_file} could not be defined due to a missing sound effects requirement!",
            Style.RESET_ALL,
        )
        print(
            f"{Style.DIM}{Fore.YELLOW}Troubleshooting:\n",
        ...
        print(
            f"{Back.BLACK}{Style.BRIGHT}{Fore.RED}FATAL ERROR:\n",
            f"{Back.RESET}",
            f"The entity in {entity_file} could not be defined due to a missing texture file!",
            Style.RESET_ALL,
        )
        print(
            f"{Style.DIM}{Fore.YELLOW}Troubleshooting:\n",
            "+Is the file named entity_name.png?\n",
            "+Is the the file saved in RP/textures/entity or RP/textures/entity/entity_name?",
            Style.RESET_ALL,
        )
        raise typer.Abort() from exc
...
def add_sounds(
    ...
    if not ce_path.exists() or not sounds_path.exists():
        raise typer.BadParameter(
            "You are missing one or more requirements in the resource pack!"
        )

    ce_data = data_from_file(ce_path)
    ce_data["minecraft:client_entity"]["description"]["sound_effects"] = (
        implement_sounds(entity_id, rp_path)
    )
